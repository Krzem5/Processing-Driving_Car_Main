class Car {
  ...
  Car(int x, int y, Track track, JSONObject nn_data) {
    this.pos=new PVector(x, y);
    this.dir=0;
    this.vel=new PVector(0, 0);
    this.acc=new PVector(0, 0);
  ...
  float[] line_collisions() {
    ...
    Line[] track_lines=this.track.get_lines();
    for (Line tl : track_lines) {
      if (collisionLineRotRect(tl, new RotRect(this.pos.x, this.pos.y, this.w, this.h, this.dir))==true) {
        noLoop();
      }
    }
    for (float a=0; a<=TWO_PI-QUARTER_PI; a+=QUARTER_PI) {
      ...
      for (Line tl : track_lines) {
        if (intersectionLineLine(tl, l)!=null&&this.pos.dist(intersectionLineLine(tl, l))<minDist) {
          intersection=intersectionLineLine(tl, l);
          minDist=this.pos.dist(intersectionLineLine(tl, l));
        }
      }
      if (minDist<LEN+1) {
        push();
  ...
  void update() {
    ...
    push();
    translate(800, 0);
    graph_nn(this.nn, data, 400, 800);
    pop();
    //if (mousePressed) {
    //  this.pos.set(mouseX, mouseY);
    //  if (!(pmouseX==mouseX&&pmouseY==mouseY)) {
    //    this.dir=this.pos.copy().sub(new PVector(pmouseX, pmouseY)).heading()+HALF_PI;
    //  }
    //}
  }
  void draw() {
